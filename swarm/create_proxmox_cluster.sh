...
remote_ssh() {
  sshpass -p "$PROXMOX_PASS" ssh -n "${PROX_SSH_OPTS[@]}" "${PROXMOX_USER}@${PROXMOX_HOST}" "LC_ALL=C LANG=C $*"
}
...
discover_mgmt_ip() {
  ...
  for ip in "${CANDIDATES[@]}"; do
    if sshpass -p "$LAB_PASS" ssh -n "${LAB_SSH_OPTS[@]}" "${LAB_USER}@${ip}" true >/dev/null 2>&1; then
      echo "$ip"
      return 0
    fi
  done
...
wait_ready() {
  ...
  while (( retries > 0 )); do
    if sshpass -p "$LAB_PASS" ssh -n "${LAB_SSH_OPTS[@]}" "${LAB_USER}@${mgmt_ip}" "LC_ALL=C LANG=C test \"\$(hostname -s)\" = \"$expected_hostname\" && for svc in pve-cluster pvedaemon pveproxy pvestatd; do systemctl is-active --quiet \"\$svc\" || exit 1; done"; then
      return 0
    fi
    retries=$((retries - 1))
    sleep 5
...
for i in "${!MGMT_IPS[@]}"; do
  ...
  sshpass -p "$LAB_PASS" ssh -n "${LAB_SSH_OPTS[@]}" "${LAB_USER}@${mgmt}" "LC_ALL=C LANG=C bash -s" -- "$host" "$HOSTS_BLOCK" <<'EOF'
set -euo pipefail
expected_hostname="$1"
hosts_block="$2"
...
CURRENT_CLUSTER_NAME="$(sshpass -p "$LAB_PASS" ssh -n "${LAB_SSH_OPTS[@]}" "${LAB_USER}@${PRIMARY_MGMT_IP}" "LC_ALL=C LANG=C pvecm status 2>/dev/null | awk -F': *' '/^Name:/{print \$2; exit}'" || true)"
if [[ -z "$CURRENT_CLUSTER_NAME" ]]; then
  log "Creating cluster ${CLUSTER_NAME} on ${PRIMARY_MGMT_IP}"
  sshpass -p "$LAB_PASS" ssh -n "${LAB_SSH_OPTS[@]}" "${LAB_USER}@${PRIMARY_MGMT_IP}" "LC_ALL=C LANG=C pvecm create ${CLUSTER_NAME} --link0 ${PRIMARY_CLUSTER_IP}"
...
for i in "${!MGMT_IPS[@]}"; do
  ...
  joined_name="$(sshpass -p "$LAB_PASS" ssh -n "${LAB_SSH_OPTS[@]}" "${LAB_USER}@${mgmt}" "LC_ALL=C LANG=C pvecm status 2>/dev/null | awk -F': *' '/^Name:/{print \$2; exit}'" || true)"
  ...
  sshpass -p "$LAB_PASS" ssh -n "${LAB_SSH_OPTS[@]}" "${LAB_USER}@${mgmt}" "LC_ALL=C LANG=C pvecm add ${PRIMARY_CLUSTER_IP} --use_ssh 1 --force --link0 address=${node_cluster_ip}"
done

EXPECTED_COUNT="${#CLUSTER_NODE_IPS[@]}"
ACTUAL_COUNT="$(sshpass -p "$LAB_PASS" ssh -n "${LAB_SSH_OPTS[@]}" "${LAB_USER}@${PRIMARY_MGMT_IP}" "LC_ALL=C LANG=C pvecm status | awk -F': *' '/^Nodes:/{print \$2; exit}'")"
...
sshpass -p "$LAB_PASS" ssh -n "${LAB_SSH_OPTS[@]}" "${LAB_USER}@${PRIMARY_MGMT_IP}" "LC_ALL=C LANG=C pvecm nodes"
